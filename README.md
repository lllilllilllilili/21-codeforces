...
🟨 : 난이도가 올라가고 후반 문제로 자주 출제됨

🟩 : 잘 출제되진 않는데 한번 나오면 어렵게 나올 수 있음

🟦 : 드물지만 특정 회사에서는 lock 걸린것처럼 매번 나오는 문제들

🟪 : 범접할 수 없는 플랜티넘 이상 급들 자주 나오는 문제
...
> - [토마토](./algorithm/BFS/토마토/BOJ_7576_토마토.java) | 백준 | Java | 동작원리, 입력받은 맵으로부터 1 이라고 써인 위치의 지점을 큐에 넣습니다. 큐에서 꺼내어 4방향(좌, 우, 위, 아래) 로 탐색을 진행합니다. 이때 주의해야할것은 1이 하나가 아니라 여러개가 있을 수 있기 때문에 탐색의 횟수를 하나의 값으로 설정하는것이 아니라, 큐에 넣는 값으로 둬야 동시에 1인 위치에서 4방향 탐색을 진행하더라도 방문체크를 해주기 때문에 끝나는 순간에 정답을 도출할 수 있다고 생각했습니다. 또 맵에서 -1인 지점은 갈수 없는 곳입니다. 따라서 이 부분도 역시 방문체크를 했습니다. (탐색에서는 진행될 수 없으므로), 시간복잡도와 공간복잡도가 서로 반비례하는 문제였습니다.
> - [가장먼노드](./algorithm/BFS/가장먼노드/가장먼노드.java) | 프로그래머스 | Java | 노드 1번에서 가장 먼 노드의 거리를 찾는 문제이다. BFS를 통해 문제를 해결할 수 있다. 개선할 방법을 찾아봤지만 좋은 방법이 떠오르지 못한 문제
...
> - [소문난칠공주](./algorithm/DFS/BOJ_1941_소문난칠공주/BOJ_1941_소문난칠공주.java) | 백준 | Java | 동작원리, DFS 접근으로 해결 할 수 있다. BOJ_Q&A 확인
> - [에너지 모으기](./algorithm/DFS/에너지모으기/BOJ_16198_에너지모으기.java) | 백준 | Java | 배열의 크기를 어떻게 둘까 고민을 많이했는데 -1 씩 감소하여 배열의 크기가 2일때까지 확인해서 해결할 수 있었다.
> - [에너지 모으기\_v2](./algorithm/DFS/에너지모으기/BOJ_16198_에너지모으기_v2.java) | 백준 | Java | 앞에 나온 문제에서 ArrayList를 이용해서 풀 수 있으나 메모리 2배, 시간은 약 40~50% 늘어나게 되더라...
...
> - [Nqueen](./algorithm/DFS/nQueen/BOJ_9663_nQueen.java) | 백준 | Java | 백트래킹 문제인데 조금 신기하다. O(N) 안에 풀 수 있다.
...
> - [새로운게임](./algorithm/구현/BOJ_17780_새로운게임/BOJ_17780_새로운게임.java) | 프로그래밍 기초를 쌓게 도와주는 시뮬레이션 문제!
> - [새로운게임2](./algorithm/구현/BOJ_17837_새로운게임2/BOJ_17837_새로운게임2.java) | 새로운게임과 달리 매번 검사해줘야 하는 문제! 차이를 너무 늦게 깨달았다.
> - [디스크컨트롤러](./algorithm/구현/프로그래머스_디스크컨트롤러/프로그래머스_디스크컨트롤러.java) |
...
> - [SWEA*모의SW역량테스트*수영장](./algorithm/구현/SWEA_모의SW역량테스트_수영장/swea_수영장.java) | DFS로 해결할 수 있는 시뮬레이션 문제
> - [SWEA*모의SW역량테스트*요리사](./algorithm/구현/SWEA_모의SW역량테스트_요리사/SWEA_모의SW역량테스트_요리사.java) | 시뮬레이션 문제이다.
> - [SWEA*모의SW역량테스트*특이한자석](./algorithm/구현/SWEA_모의SW역량테스트_특이한자석.java) | 특이한 자석 문제, 비슷한 문제가 있었던것 같기도 하고
...
> - [원판 돌리기](./algorithm/구현/원판 돌리기/BOJ*17822*원판돌리기.java) | 시뮬레이션 문제
...
> - [음양더하기](./algorithm/구현/프로그래머스_음양더하기/프로그래머스_음양더하기.java) | 프로그래머스 | Java | absolutes 길이가 1이상 1,000이하 밖에 안되서 for문 한번 순회로 O(N)으로 문제를 해결할 수 있었습니다.
> - [키패드누르기](./algorithm/구현/프로그래머스_키패드누르기/프로그래머스_키패드누르기.java) | 프로그래머스 | Python | 왼손, 오른손으로 누를 때 기준을 두고 2,4,8,0 일 경우 왼손, 오른손이 가리키고 있는 거리를 구해서 더 짧은 쪽으로 갱신합니다.
> - [N진수 게임](./algorithm/구현/KAKAO_n진수게임/KAKAO_n진수게임.py) | 프로그래머스 | Python | 16진수까지 숫자가 나올 수 있기 때문에 가능한한 0~9 + A~Z 더한 숫자를 구해 몫/나머지를 구하여 t\*m 개수 만큼 문자열을 만든 후 m 횟수만큼 자릿수 문자를 조합하여 정답을 출력
...
> - [예산](./algorithm/정렬/SUMMER/WINTER_예산/SUMMER/WINTER_예산.java) | 단순히 정렬 후 조건에 맞게 구현해주면 되었습니다.
...
> - [122. Best Time to Buy and Sell Stock II](./algorithm/그리디알고리즘/122.BestTimetoBuyandSellStockll/BestTimetoBuyandSellStockll.java) | 2개로 접근할 수 있는데 `브루트포스`, `그리디` 스럽게 접근할 수 있는데 후자의 방법으로 해결해보았습니다.
...
> - [앱](./algorithm/Dynamic/BOJ_7579_앱.py) | 냅색을 활용한 문제입니다.
...
> - [배달](./algorithm/다익스트라/프로그래머스_배달/프로그래머스_배달.java) | 백준 | Java | 이차원 맵이 아닌 ArrayList를 활용한 다익스트라 풀이
...
> - [랜선자르기](./algorithm/이분탐색/BOJ_1654_랜선자르기/BOJ_1654_랜선자르기.java) | 백준 | Java | O(logN) | 찾고자 하는 랜선의 길이를 이분탐색으로 처리
...
> - [줄세우기](./algorithm/정렬/BOJ_2252_줄세우기/BOJ_2252_줄세우기.java) | 백준 | Java | 위상정렬 문제중 가장 좋은 표본이 되는 문제였다.
